teams = 'Flamengo', 'Internacional', 'Atlético-MG', 'São Paulo','Fluminense', 'Grêmio', 'Palmeiras', 'Santos', 'Athlétcio-PR', 'Bragantino', 'Ceará', 'Corinthians', 'Atlético-GO', 'Bahia', 'Sport', 'Fortaleza', 'Vasco', 'Goiás', 'Coritiba', 'Botafogo'

print(f'Colocações Finais do Brasileirão: {teams}')
print('-='*30)
print(f'Os 5 primeiros foram {teams[0:5]}')
print('-='*30)
print(f'Os 4 últimos colocados foram{teams[-4:]}')
print('-='*30)
print(f'Ordem Alfabética {sorted(teams)}')
print('-='*30)
print('O Santos está na {}ª posição'.format(teams.index('Santos')+1))
